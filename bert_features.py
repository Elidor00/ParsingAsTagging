...
class Bert(object):
    ...
    def __init__(self, layer_indexes, max_seq_length, batch_size, multi_lingual=False, which_cuda=0):
        ...
        self.layer_indexes = [int(x) for x in layer_indexes.split(",")]
        # tokenizer lower case for english, keep case for multi lingual
        # self.tokenizer = BertTokenizer.from_pretrained(pretrained_model, do_lower_case=not multi_lingual)
    ...
    def extract_bert_features(self, conll_dataset):
        ...
        for input_ids, input_mask, token_maps, example_indices in tqdm(eval_dataloader, desc="Extracting..."):
            ...
            # averaged_output = torch.stack([all_encoder_layers[idx] for idx in self.layer_indexes]).mean(0) \
            #                   / len(self.layer_indexes)
